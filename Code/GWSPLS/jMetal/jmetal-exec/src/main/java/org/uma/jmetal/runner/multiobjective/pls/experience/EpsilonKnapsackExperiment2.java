...
/**
 * 注意设为50!!!!!!!!!!!
 * epsilons = {0,0,5,70,200,400,600}; 6 dimension 旧数据
 * epsilons = {0,0,5,70,150,200,300}; 4,5dimension 旧数据
 * @author sevn
 *
 */
public class EpsilonKnapsackExperiment2 {
	public static void main(String[] args) throws IOException {
		double[] epsilons = {0,0,5,70,200,400,600};
		int[] setPopSize = {0,0,300,300,364,495,792};
		int[] itemNumbers = { 250, 500, 750 };
		String baseDirectory = "results";
		for (int i = 0; i < itemNumbers.length; i++) {
			for (int dimension = 4; dimension < 7; dimension++) {
				for (int run = 0; run < 30; run++) {
					...
					Algorithm<List<PermutationSolution<Integer>>> algorithm = 
							...
							.setEpsilon(epsilons[dimension])
							.setInitializeSolutionMethod(InitialSolutionMethod.GREEDY_METHOD)
							.setMaxPopulationSize(setPopSize[dimension]).
							setMaxIteration(100).
