...
public class ManyObjectiveTSP extends AbstractIntegerPermutationProblem{

	protected int numberOfCities;
	protected List<double[][]> objectiveMatrics = new ArrayList<double[][]>();
	
	public ManyObjectiveTSP(String[] tspPaths, String name) {
		try {
			for (int i = 0; i < tspPaths.length; i++) {
				double[][] objectiveMatrix;

				objectiveMatrix = readProblem(tspPaths[i]);
